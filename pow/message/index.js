        ...
        switch (command) {
            ...
            case 'register':
                ...
                if (isGroupMsg) {
                    ...
                    await bocchi.sendText(sender.id, eng.registered(namaUser, umurUser, sender.id, time, serialUser))
            ...
            case 'level':
                ...
                if (userLevel === undefined && userXp === undefined) return await bocchi.reply(from, eng.levelNull(), id)
            ...
            case 'setbg':
                ...
                if (levels === undefined && xps === undefined) return await bocchi.reply(from, eng.levelNull(), id)
            ...
            case 'joox':
                ...
                downloader.joox(q)
                    .then(async ({ result }) => {
                        await bocchi.sendFileFromUrl(from, result[0].linkImg, `${result[0].judul}.jpg`, eng.joox(result), id)
            ...
            case 'menu':
                ...
                if (args[0] === '1') {
                    ...
                    await bocchi.sendText(from, eng.menu())
                }
            break
            case 'rules':
            case 'rule':
            ...
            case 'report':
                ...
                if (lastReport !== undefined && cd - (Date.now() - lastReport) > 0) {
                    ...
                    await bocchi.reply(from, eng.limit(time), id)
                } else {
                    if (isGroupMsg) {
            ...
            case 'tos':
                if (!isRegistered) return await bocchi.reply(from, eng.notRegistered(), id)
                await bocchi.sendLinkWithAutoPreview(from, 'http://powsitos.com', eng.tos(ownerNumber))
            break
            case 'join':
            ...
            case 'me':
                ...
                if (quotedMsg) {
                    ...
                    await bocchi.sendFileFromUrl(from, pfp, `${username}.jpg`, eng.profile(username, status, premi, benet, adm), id)
                } else {
            ...
            case 'sfire':
                ...
                if (isMedia && isImage || isQuotedImage) {
                    ...
                    await bocchi.reply(from, eng.wrongFormat(), err)
                }
            break
            case 'ttg':
                if (!isRegistered) return await bocchi.reply(from, eng.notRegistered(), id)
            ...
            case 'nh':
                if (!isRegistered) return await bocchi.reply(from, eng.notRegistered(), id)
                const kode = args[0]
                if (!kode) return await bocchi.reply(from, eng.wrongFormat(), id)
                if (isGroupMsg) {
                    if (!isNsfw) return await bocchi.reply(from, eng.notNsfw(), id)
                    await bocchi.reply(from, eng.wait(), id)
            ...
            case 'hakai':
                ...
                for (let i of mentionedJidList) {
                    if (groupAdmins.includes(i)) return bocchi.reply(from, 'Ahorita no joven!', id)
            ...
            case 'premium':
                ...
                if (ar[0] === 'add') {
                    for (let premi of mentionedJidList) {
                        if (premi === botNumber) return await bocchi.reply(from, eng.wrongFormat(), id)
                        _premium.push(premi)
            ...
            case 'setstat':
                ...
                await bocchi.sendText(from, eng.doneOwner())
            break
            case 'kickall':
			if (!isRegistered) return await bocchi.reply(from, eng.notRegistered(), id)
            if (!isGroupMsg) return await bocchi.reply(from, eng.groupOnly(), id)
            ...
            for (let i = 0; i < allMem.length; i++) {
                if (groupAdmins.includes(allMem[i].id)) {
                    console.log('Upss es Admin')
                } else {
                    await bocchi.removeParticipant(groupId, allMem[i].id)
					await bocchi.reply(from, 'Listo ... Eliminados!', id)
                }
            }
            break
			case 'radio':
			if (!isRegistered) return await bocchi.reply(from, eng.notRegistered(), id)
					await bocchi.reply(from, 'Sintonizanos en http://powsitos.com/radio! \n\n*Powsitos Radio*', id)
            break
			case 'bot':
			if (!isRegistered) return await bocchi.reply(from, eng.notRegistered(), id)
					await bocchi.reply(from, 'Tes chingando .. ! \n\n*-.-*', id)
            break
			case 'reglasventas':
			if (isGroupMsg) return await bocchi.reply(from, 'Grupo de Ventas y Servicios\n\n✅ Anuncia tu producto o servicio, sin ser repetitivo\n✅ Si vendes un producto, especifica precios\n❌ No groserías, ni insultos\n❌ No sexo servicios\n❌ No ventas / Ni compras de animales\n❌ No es grupo de chisme ni charla\n❌ No envíes enlaces de whatsapp o facebook sin anunciar tus productos aquí en el grupo y especifica de que trata tu grupo.\n\nTelegram : https://t.me/ventascardel', id)
            break
            case 'serial':
                if (!isOwner) return await bocchi.reply(from, eng.ownerOnly(), id)
                if (args.length !== 1) return await bocchi.reply(from, eng.wrongFormat(), id)
            ...
            default:
                if (isCmd) {
                    await bocchi.reply(from, `Comando no encontrado *${prefix}${command}* .`, id)
                }
            break
        }
    } catch (err) {
